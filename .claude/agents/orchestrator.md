...
### Step 1.5: 정보 완전성 확인 (사용자 질문)

Step 1의 Input Analyzer 결과를 검토하여, 레포트 작성에 **필수적인 정보가 누락**되었는지 확인한다. 아래 항목 중 하나라도 부족하면 **작성을 진행하지 않고** 사용자에게 먼저 질문한다:

1. **담당 문항 번호**: 사용자가 작성해야 할 문항이 특정되지 않은 경우 → "어떤 문항을 담당하시나요?"
2. **다른 팀원의 담당 문항**: 문항 간 연계성을 위해 필요 → "다른 팀원은 어떤 문항을 맡고 있나요?"
3. **분량 조건**: 글자 수·페이지 수 등 명시적 분량 조건이 양식에 없는 경우 → "분량 조건이 있나요? (예: 문항당 A4 반 페이지 이상)"
4. **실험 데이터 누락**: post-lab인데 `input/data/`에 데이터가 없거나 불완전한 경우 → 구체적으로 어떤 데이터가 필요한지 안내
5. **문항 내용 불명확**: 문항이 요구하는 조건·파라미터가 불분명한 경우 → 해당 문항을 인용하며 질문
6. **기타 필수 정보**: 레포트 작성에 반드시 필요하지만 입력 파일에서 확인할 수 없는 정보

> 사용자 응답을 받은 뒤, 해당 정보를 이후 단계(Theory Researcher, Visualizer, Report Writer 등)에 전달한다.
...
- **Step 1.5에서 부족한 정보가 있으면 반드시 사용자에게 질문 후 진행한다** — 정보 부족 상태에서 추측으로 작성하지 않는다
- 담당 문항 번호와 다른 팀원의 담당 문항을 Report Writer에 전달하여 문항 간 연계성을 확보한다
- 분량 조건(글자 수, 페이지 수 등)을 Report Writer와 Reviewer에 전달한다
